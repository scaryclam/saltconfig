...
def _createdb_cmd(db_name, *args, **kwargs):
    '''
    Return string with fully composed createdb command.
    
    This is an alternative to the _psql_cmd.
    '''
...
def db_create(name,
    ...
    '''
    if run_createdb:
        cmd = _createdb_cmd(name, user=user, host=host, port=port,
                            encoding=encoding, passwd=passwd, template=template,
                            owner=owner)
    else:
